#!/usr/bin/env bash
# ==============================================================================
# System Monitor Widget (com.mrudhuhas.systemmonitor) - Plasma 6 Installer
...
echo -e "${CYAN}====================================================${NC}"
echo -e "${CYAN}  Installing KDE Plasma 6 System Monitor Widget      ${NC}"
...
# Method 1: Using kpackagetool6
if command -v kpackagetool6 &> /dev/null; then
    if [ "$CLEAN_INSTALL" = true ]; then
        echo -e "${YELLOW}--> Performing clean reinstallation (--clean)...${NC}"
        if kpackagetool6 -t Plasma/Applet --list | grep -q "${PLUGIN_ID}"; then
            echo -e "${YELLOW}--> Removing previous plasmoid registration...${NC}"
            kpackagetool6 -t Plasma/Applet --remove "${PLUGIN_ID}" || true
        fi
        if [ -d "${DEST_DIR}" ]; then
            echo -e "${YELLOW}--> Removing old directory: ${DEST_DIR}...${NC}"
            rm -rf "${DEST_DIR}"
        fi
        echo -e "${GREEN}--> Installing fresh plasmoid package...${NC}"
    ...
    # Method 2: Direct user directory deployment
...
echo -e "\n${GREEN}✔ System Monitor installation / upgrade complete!${NC}"
echo -e "${CYAN}----------------------------------------------------${NC}"
echo -e "Next steps on Fedora 44 / KDE Plasma 6:"
echo -e "  1. Right click your desktop -> ${YELLOW}Add Widgets...${NC}"
echo -e "  2. Search for ${YELLOW}'System Monitor'${NC}"
echo -e "  3. Drag and drop it onto your desktop."
echo -e "  4. Right click the widget -> ${YELLOW}Configure System Monitor...${NC} to customize."
echo -e ""
